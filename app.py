...
# --- 3. FUNKCJE DANYCH (IZOLACJA ARKUSZY) ---
def load_targi(u):
...
if menu == "🏠 DZIENNIK":
    ...
    with st.expander("➕ NOWY MELDUNEK"):
        with st.form("new_entry_form", clear_on_submit=True):
            ...
            if st.form_submit_button("ZATWIERDŹ"):
                ...
                st.success("DODANO DO TWOJEGO REJESTRU.")
                time.sleep(1)
                st.rerun()
    ...
    if not my_data.empty:
        edited_df = st.data_editor(my_data, use_container_width=True, hide_index=True, column_config=col_config, num_rows="dynamic", key=f"editor_{user}")
...
# --- MODUŁ 4: ROZKAZY ---
    ...
    with t2:
        if not df_zadania.empty:
            edited_tasks = st.data_editor(df_zadania, use_container_width=True, hide_index=True, num_rows="dynamic")
