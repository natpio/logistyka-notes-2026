...
# --- 1. KONFIGURACJA WIZUALNA: WOJSKOWY REJESTR OPERACYJNY ---
...
st.markdown("""
    ...
    /* Kontenery jak pożółkłe arkusze papieru */
    ...
    /* Przyciski - czerwone pieczątki zatwierdzające */
    .stButton>button {
        background-color: #fdf5e6; 
        color: #8b0000; 
...
# --- 2. BAZA STAWEK EKSPEDYCYJNYCH 2026 ---
EXP_RATES = {
...
# --- 3. POŁĄCZENIE I AUTORYZACJA ---
conn = st.connection("gsheets", type=GSheetsConnection)
st.sidebar.markdown("<h2 style='text-align: center; color: #fdf5e6;'>🗄️ REJESTR SZTABOWY</h2>", unsafe_allow_html=True)
...
if user != "Wybierz...":
    ...
    elif input_pin: st.sidebar.error("🚨 KOD BŁĘDNY. NARUSZENIE PROCEDURY!")

if not is_authenticated:
    st.info("Oczekiwanie na autoryzację urzędnika logistyki...")
    st.stop()

# --- 4. DANE (ZGODNIE Z TWOIM PLIKIEM) ---
try:
    ...
    st.error("Błąd łączności z centralą (GSheets).")
    st.stop()

# --- 5. MENU REJESTRÓW ---
menu = st.sidebar.radio("PROTOKÓŁ:", ["🏠 DZIENNIK OPERACJI", "📅 KALENDARIUM", "📊 WYKRES GANTA", "📋 TABLICA ROZKAZÓW"])
...
if menu == "🏠 DZIENNIK OPERACJI":
    ...
    with st.expander("🧮 Kalkulator Norm Zaopatrzenia 2026", expanded=True):
        ...
        if calc:
            st.markdown(f'<div class="recommendation-box"><b>MELDUNEK:</b> {calc["name"]} | <b>€ {calc["cost"]:.2f}</b><br>{calc["uk_info"]}</div>', unsafe_allow_html=True)

    active_mask = df_all["Status"] != "WRÓCIŁO"
    active_df = df_all[active_mask].copy()
    
    st.subheader(f"✍️ Rejestr Osobisty Oficera: {user}")
    ...
    edited_my = st.data_editor(my_tasks, use_container_width=True, hide_index=True, column_config=col_config, key="ed_ops")
    ...
    if st.button("💾 ZAPISZ I ZALAKUJ AKTA"):
        ...
        st.cache_data.clear(); st.success("Akta zaktualizowane."); st.rerun()
...
# --- MODUŁ 3: WYKRES GANTA (PEŁNA LOGIKA) ---
elif menu == "📊 WYKRES GANTA":
    st.title("📊 Harmonogram Operacyjny (Gantt)")
...
# --- MODUŁ 4: TABLICA ROZKAZÓW (TWOJA LOGIKA ARCHIWIZACJI) ---
    ...
    st.title("📋 Meldunki i Czyny Społeczne")
    ...
    if st.button("💾 ZAKTUALIZUJ TABLICĘ I ARCHIWIZUJ"):
        new_my = edited_n.copy()
        new_my["Autor"] = user
        # TWOJA LOGIKA: Automatyczna data dla wykonanych
        ...
        st.cache_data.clear(); st.success("Tablica zsynchronizowana."); st.rerun()
