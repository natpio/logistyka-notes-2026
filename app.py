...
def darmowy_agent_logistyczny(transcript, df):
    ...
    ZADANIE:
    1. Jeśli mowa o edycji: Zwróć JSON: {{"akcja": "edytuj", "nazwa": "nazwa", "pole": "Status", "wartosc": "WRÓCIŁO"}}
    2. Jeśli mowa o dodaniu: Zwróć JSON: {{"akcja": "dodaj", "dane": {{"Nazwa Targów": "Berlin", "Pierwszy wyjazd": "2026-05-10", "Data końca": "2026-05-15", "Status": "OCZEKUJE"}}}}
    Zwróć TYLKO czysty JSON.
    """
    try:
        chat_completion = client.chat.completions.create(messages=[{"role": "user", "content": prompt}], model="llama3-70b-8192", response_format={ "type": "json_object" })
...
if "GROQ_API_KEY" not in st.secrets:
    st.sidebar.warning("Brak klucza GROQ_API_KEY.")
else:
...
if menu == "🏠 DZIENNIK OPERACJI":
    ...
    with st.expander("🧮 Kalkulator Norm Zaopatrzenia 2026", expanded=False):
        ...
        if calc:
            uk_html = f"<div class='uk-alert'><b>Doliczono koszty UK:</b><br>{calc['uk_info']}</div>" if calc['uk_info'] else ""
            st.markdown(f"<div class='recommendation-box'><b>MELDUNEK:</b> Rekomendowany transport: {calc['name']}<br><b>KOSZT SZACUNKOWY:</b> <span style='font-size: 1.3rem;'>€ {calc['cost']:.2f} netto</span>{uk_html}</div>", unsafe_allow_html=True)

    st.markdown("---")
    active_df = df_all[df_all["Status"] != "WRÓCIŁO"].copy()
    col_config = {"Status": st.column_config.SelectboxColumn("Status", options=["OCZEKUJE", "W TRAKCIE", "WRÓCIŁO", "ANULOWANE"]), "Logistyk": st.column_config.SelectboxColumn("Logistyk", options=["DUKIEL", "KACZMAREK"]), "Sloty": st.column_config.SelectboxColumn("Sloty", options=["TAK", "NIE", "NIE POTRZEBA"]), "Pierwszy wyjazd": st.column_config.DateColumn("Start"), "Data końca": st.column_config.DateColumn("Powrót")}
    
    st.subheader(f"✍️ TWOJA SEKCJA: {user}")
    search_me = st.text_input("🔍 Szukaj w swoich projektach:").lower()
    ...
    edited_my = st.data_editor(my_tasks, use_container_width=True, hide_index=True, column_config=col_config, num_rows="dynamic")
