...
user = st.sidebar.selectbox("👤 Tożsamość:", ["Wybierz...", "DUKIEL", "KACZMAREK"])
...
if user != "Wybierz...":
    ...
    if input_pin == user_pins.get(user):
        ...
        st.sidebar.error("❌ ODMOWA DOSTĘPU - NIEPOPRAWNY KOD")
...
# --- 5. MENU ---
menu = st.sidebar.radio("ZAKRES DZIAŁAŃ:", ["🏠 DYSPOZYTURA POLOWA", "📅 HARMONOGRAM OPERACJI", "📊 WYKRESY GOTOWOŚCI", "📋 TABLICA ROZKAZÓW"])

# --- MODUŁ 1: DYSPOZYTURA POLOWA ---
if menu == "🏠 DYSPOZYTURA POLOWA":
    ...
    with st.expander("🧮 Obliczenia Logistyczne (Normy 2026)", expanded=True):
        ...
        t_city = c1.selectbox("Kierunek (Rejon działań):", sorted(list(EXP_RATES["WŁASNY SQM BUS"].keys())))
        t_weight = c2.number_input("Ciężar zaopatrzenia (kg):", min_value=0, value=500, step=100)
        t_start = c3.date_input("Data mobilizacji:", datetime.now())
        t_end = c4.date_input("Data demobilizacji:", datetime.now() + timedelta(days=4))
    ...
    st.subheader(f"✍️ Rejestr Aktywnych Operacji (Oficer: {user})")
    ...
    col_config = {
        ...
        "Sloty": st.column_config.SelectboxColumn("Status Slotu", options=["TAK", "NIE", "NIE POTRZEBA"]),
    ...
    if st.button("💾 ZAPISZ RAPORT I AKTUALIZUJ STATUS"):
        ...
        st.cache_data.clear()
        st.success("Raport wysłany do dowództwa. Operacje zakończone przeniesiono do archiwum.")
        st.rerun()

    st.markdown("---")
    partner = "KACZMAREK" if user == "DUKIEL" else "DUKIEL"
    st.subheader(f"👁️ Monitorowanie Sekcji Sąsiedniej (Oficer: {partner})")
...
# --- MODUŁ 2: HARMONOGRAM OPERACJI ---
    ...
    for _, r in df_all[(df_all["Status"] != "WRÓCIŁO") & (df_all["Pierwszy wyjazd"].notna())].iterrows():
        color = "#8b0000" if r["Logistyk"] == "DUKIEL" else "#1a1c0a"
        events.append({
...
# --- MODUŁ 3: WYKRESY GOTOWOŚCI ---
    ...
    if not df_viz.empty:
        fig = px.timeline(df_viz, x_start="Pierwszy wyjazd", x_end="Data końca", y="Nazwa Targów", 
                          color="Logistyk", color_discrete_map={"DUKIEL": "#8b0000", "KACZMAREK": "#1a1c0a"},
                          template="plotly_white")
        fig.update_yaxes(autorange="reversed")
        fig.update_layout(paper_bgcolor="#4b5320", plot_bgcolor="#3e441c", font_color="#d4d7b3")
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Brak aktywnych transportów w polu widzenia.")
...
# --- MODUŁ 4: TABLICA ROZKAZÓW ---
    ...
    with c1:
        ...
        for _, t in df_notes[df_notes["Status"] == "DO ZROBIENIA"].iterrows():
            st.markdown(f"<div class='task-card' style='border-left-color: #8b0000'><b>{t.get('Tytul', 'Zadanie')}</b><br><small>Zgłosił: {t['Autor']}</small></div>", unsafe_allow_html=True)
    with c2:
        st.markdown("### 🟡 W TOKU")
        for _, t in df_notes[df_notes["Status"] == "W TRAKCIE"].iterrows():
            st.markdown(f"<div class='task-card' style='border-left-color: #1a1c0a'><b>{t.get('Tytul', 'Zadanie')}</b><br><small>Zgłosił: {t['Autor']}</small></div>", unsafe_allow_html=True)

    st.markdown("---")
    st.subheader("🖋️ Aktualizacja Dziennika Rozkazów")
    ...
    if st.button("💾 ZAPISZ W DZIENNIKU BOJOWYM"):
        ...
        st.cache_data.clear()
        st.success("Tablica rozkazów zaktualizowana.")
        st.rerun()
